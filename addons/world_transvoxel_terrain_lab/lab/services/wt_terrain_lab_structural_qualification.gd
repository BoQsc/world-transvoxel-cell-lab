...
static func run() -> Dictionary:
	...
	return {
		...
		"scope_status": {
			"TQP-66": "implemented_pending_strength_and_under_resolution_matrix",
			"TQP-67": "implemented_ownership_only_pending_fluid_system_evidence",
			"TQP-68": "implemented_pending_collision_destruction_and_persistence",
			"TQP-69": "implemented_pending_bridge_authoring_and_lod_matrix",
			"TQP-70": "implemented_pending_rigid_body_remesh_collision_and_persistence",
		},
		"implemented_scope": [
		...
		"explicitly_unqualified_scope": [
			...
			"complete TQP-66 through TQP-70 qualification",
		],
...
static func _qualify_connectivity() -> Dictionary:
	...
	var result := _result("TQP-66", solid.size() + 4, failures)
...
static func _qualify_collapse_policy() -> Dictionary:
	...
	var result := _result("TQP-70", components.size() + 1, failures)
...
static func _qualify_fluid_ownership() -> Dictionary:
	...
	var result := _result("TQP-67", ownership.size() + 1, failures)
...
static func _qualify_placement_invalidation() -> Dictionary:
	...
	var result := _result("TQP-68", first.size() + 2, failures)
...
static func _qualify_authored_composition() -> Dictionary:
	...
	var result := _result("TQP-69", 7, failures)

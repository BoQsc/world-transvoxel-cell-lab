...
const Qualification := preload(
	"res://addons/world_transvoxel_terrain_lab/lab/services/wt_terrain_lab_independent_oracle_qualification.gd"
)
...
func _run() -> void:
	...
	if fixture_filter.is_empty() and not candidate:
	...
	if str(result.get("status", "")) != "PASS":
		_fail("independent-oracle validation failed: " + str(result.get("failures", [])))
		return
	print("WT_TERRAIN_LAB_INDEPENDENT_ORACLE_PASS")
	quit(0)
...
func _fail(message: String) -> void:
	push_error("WT_TERRAIN_LAB_INDEPENDENT_ORACLE_FAIL: " + message)
	quit(1)

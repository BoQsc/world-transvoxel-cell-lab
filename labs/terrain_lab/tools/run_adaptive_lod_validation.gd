...
const Qualification := preload(
	"res://addons/world_transvoxel_terrain_lab/lab/services/wt_terrain_lab_adaptive_lod_qualification.gd"
)
...
func _run() -> void:
	...
	var scenario_filter := _argument_value("--scenario")
	var result := Qualification.run(scenario_filter)
	...
	if str(result.get("status", "")) != "PASS":
		_fail("adaptive LOD validation failed: " + str(result.get("failures", [])))
		return
	print("WT_TERRAIN_LAB_ADAPTIVE_LOD_PASS")
	quit(0)
...
func _fail(message: String) -> void:
	push_error("WT_TERRAIN_LAB_ADAPTIVE_LOD_FAIL: " + message)
	quit(1)

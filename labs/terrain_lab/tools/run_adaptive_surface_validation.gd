...
const Qualification := preload(
	"res://addons/world_transvoxel_terrain_lab/lab/services/wt_terrain_lab_adaptive_surface_qualification.gd"
)
...
func _run() -> void:
	...
	if not output_path.is_empty():
		...
		if file == null:
			push_error("WT_TERRAIN_LAB_ADAPTIVE_SURFACE_WRITE_FAIL " + output_path)
			quit(2)
			return
		file.store_string(JSON.stringify(report, "\t", false) + "\n")
		file.close()
	if str(report.get("status", "")) != "PASS":
		push_error("WT_TERRAIN_LAB_ADAPTIVE_SURFACE_FAIL " + JSON.stringify(report.get("failures", [])))
		quit(1)
		return
	print("WT_TERRAIN_LAB_ADAPTIVE_SURFACE_PASS signature=" + str(report.get("semantic_signature", "")))
	quit(0)

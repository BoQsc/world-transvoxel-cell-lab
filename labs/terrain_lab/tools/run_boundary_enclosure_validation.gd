...
const Qualification := preload(
	"res://addons/world_transvoxel_terrain_lab/lab/services/wt_terrain_lab_boundary_enclosure_qualification.gd"
)
...
func _run() -> void:
	...
	if str(result.get("status", "")) != "PASS":
		_fail("boundary/enclosure validation failed: " + str(result.get("failures", [])))
		return
	print("WT_TERRAIN_LAB_BOUNDARY_ENCLOSURE_PASS")
	quit(0)
...
func _fail(message: String) -> void:
	push_error("WT_TERRAIN_LAB_BOUNDARY_ENCLOSURE_FAIL: " + message)
	quit(1)

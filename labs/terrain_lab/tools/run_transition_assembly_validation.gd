...
const Qualification := preload(
	"res://addons/world_transvoxel_terrain_lab/lab/services/wt_terrain_lab_transition_assembly_qualification.gd"
)
...
func _run() -> void:
	...
	if str(result.get("status", "")) != "PASS":
		_fail("transition assembly validation failed: " + str(result.get("failures", [])))
		return
	print("WT_TERRAIN_LAB_TRANSITION_ASSEMBLY_PASS")
	quit(0)
...
func _fail(message: String) -> void:
	push_error("WT_TERRAIN_LAB_TRANSITION_ASSEMBLY_FAIL: " + message)
	quit(1)

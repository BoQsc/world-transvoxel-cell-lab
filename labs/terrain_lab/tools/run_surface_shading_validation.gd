...
func _run() -> void:
	...
	var report := {
		"schema": "world_transvoxel.terrain_lab.surface_shading_contract_qualification.v1",
	...
	if str(report.get("status", "")) != "PASS":
		_fail("surface shading contract failed: " + str(report.get("failures", [])))
		return
	print("WT_TERRAIN_SURFACE_SHADING_CONTRACT_PASS")
	quit(0)
...
func _fail(message: String) -> void:
	push_error("WT_TERRAIN_SURFACE_SHADING_CONTRACT_FAIL: " + message)
	quit(1)

...
const Qualification := preload(
	"res://addons/world_transvoxel_terrain_lab/lab/services/wt_terrain_lab_dynamic_lod_publication_qualification.gd"
)
...
func _run() -> void:
	...
	if str(result.get("status", "")) != "PASS":
		_fail("dynamic LOD publication validation failed: " + str(result.get("failures", [])))
		return
	print("WT_TERRAIN_LAB_DYNAMIC_LOD_PUBLICATION_PASS")
	quit(0)
...
func _fail(message: String) -> void:
	push_error("WT_TERRAIN_LAB_DYNAMIC_LOD_PUBLICATION_FAIL: " + message)
	quit(1)

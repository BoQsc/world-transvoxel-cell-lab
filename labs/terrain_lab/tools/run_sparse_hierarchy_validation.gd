...
func _run() -> void:
	...
	if not output_path.is_empty():
		...
		if file == null:
			_fail("could not write TQP-42 report")
			return
		file.store_string(JSON.stringify(report, "\t", false) + "\n")
	if str(report.get("status", "")) != "PASS":
		_fail("TQP-42 qualification failed: " + str(report.get("failures", [])))
		return
	print("WT_TERRAIN_LAB_SPARSE_HIERARCHY_PASS signature=" + str(report.get("semantic_signature", "")))
	quit(0)
...
func _fail(message: String) -> void:
	push_error("WT_TERRAIN_LAB_SPARSE_HIERARCHY_FAIL " + message)
	quit(1)

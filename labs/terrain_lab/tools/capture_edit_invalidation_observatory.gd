...
const SCENE_PATH := "res://labs/terrain_lab/scenes/edit_invalidation_observatory.tscn"
const DEFAULT_OUTPUT := (
	"res://labs/terrain_lab/results/edit_invalidation_motion_windows.json"
)
const ACTIONS := [
	"mixed_lod_transition_dependency",
	"disjoint_batch_union",
	"rapid_supersession",
]
...
func _run() -> void:
	...
	if packed == null:
		_fail("edit invalidation observatory scene could not load")
		return
	...
	for action in ACTIONS:
		...
		if surface_samples < 250:
			...
			return
		var path := "res://labs/terrain_lab/results/edit_invalidation_%s_windows.png" % action
		var error := image.save_png(path)
		if error != OK:
			_fail("could not save edit invalidation capture: " + error_string(error))
			return
		captures.append({
			"action": action,
			"path": path,
	...
	if str(shutdown.get("status", "")) != "PASS":
		_fail("edit invalidation observatory did not shut down cleanly")
		return
	var report := {
		"schema": "world_transvoxel.terrain_lab.edit_invalidation_motion.v1",
		"milestone": "TQP-36",
		"status": "PASS",
		"scene": SCENE_PATH,
	...
	if file == null:
		_fail("could not open edit invalidation motion output")
		return
	file.store_string(JSON.stringify(report, "\t", false) + "\n")
	print("WT_TERRAIN_EDIT_INVALIDATION_CAPTURE_PASS " + output_path)
	quit(0)
...
func _fail(message: String) -> void:
	push_error("WT_TERRAIN_EDIT_INVALIDATION_CAPTURE_FAIL: " + message)
	quit(1)
